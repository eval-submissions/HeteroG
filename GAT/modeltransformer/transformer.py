...
from transutils import BaseModelMixin, REPO_ROOT
...
class Transformer(BaseModelMixin):
    ...
    def _positional_encoding_sinusoid(self, inp):
        ...
        with tf.variable_scope('positional_sinusoid'):
            # Copy [0, 1, ..., `inp_size`] by `batch_size` times => matrix [batch, seq_len]
            aaa = tf.placeholder(shape=[None,1],dtype=tf.float32)
            pos_ind = tf.tile(tf.expand_dims(tf.range(seq_len), 0), tf.shape(aaa))
    ...
    def construct_autoregressive_mask(self, target):
        ...
        aaa= tf.placeholder(shape = [None,1,1],dtype=tf.float32)
        mask = tf.convert_to_tensor(tri_matrix, dtype=tf.float32)
        masks = tf.tile(tf.expand_dims(mask, 0), tf.shape(aaa))  # copies
        return masks
