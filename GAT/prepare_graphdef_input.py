...
def model_fn(model_name,batch_size):
    if model_name=="vgg19":
        ...
        net = slim.max_pool2d(net, [2, 2], 2,trainable=True)
        net = slim.conv2d(net, 64, [5, 5],trainable=True)
        net = slim.max_pool2d(net, [2, 2], 2,trainable=True)
        net = slim.flatten(net)
        net = slim.fully_connected(net, 1024, activation_fn=tf.nn.sigmoid,trainable=True)
        net = slim.fully_connected(net, 1000, activation_fn=None,trainable=True)
