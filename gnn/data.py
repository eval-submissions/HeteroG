...
def gen_topo(devices, inter=2810, intra=2810):
    ...
    features = [[time_ratio, intra] for _, time_ratio in devices] # how to encode the edge features?
...
def gen_data(gdef, prof_data, topo):
    ...
    g.add_edges(g.nodes(), g.nodes()) # self-loops are required for GAT
    prof_data = { key: [int(np.mean(times) * time_ratio) for _, time_ratio in topo["devices"]] for key, times in prof_data.items() }
    return {
        "gdef": gdef,
        "prof_data": prof_data,
        "computation_graph": g,
...
def get_all_data():
    ...
    topos1 = [gen_topo([
        ...
        ("/job:worker/replica:0/task:0/device:GPU:4", 1.2),
        ("/job:worker/replica:0/task:0/device:GPU:5", 1.2),
        ("/job:worker/replica:0/task:0/device:GPU:6", 1.2),
        ("/job:worker/replica:0/task:0/device:GPU:7", 1.2),
    ...
    topos3 = [gen_topo([
        ("/job:worker/replica:0/task:0/device:GPU:0", 1),
        ("/job:worker/replica:0/task:0/device:GPU:1", 1),
        ("/job:worker/replica:0/task:0/device:GPU:2", 1.2),
        ("/job:worker/replica:0/task:0/device:GPU:3", 1.2),
        ("/job:worker/replica:0/task:0/device:GPU:4", 1.5),
        ("/job:worker/replica:0/task:0/device:GPU:5", 1.5),
