...
with tf.device("/gpu:0"):
    ...
    for epoch in range(20000):
        ...
        strategy = np.zeros((cnfeats.shape[0], tnfeats.shape[0]), dtype=np.float32)
        ...
        with tf.GradientTape() as tape:
            ...
            placement_logit = model([cnfeats, cefeats, cntypes, tnfeats, tefeats, strategy, None], training=True)
            # loss = 0
            # for _ in range(4):
            ...
            loss = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(strategy, placement_logit))
            info(loss.numpy())
